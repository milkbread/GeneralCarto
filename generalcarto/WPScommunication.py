...
from generalcarto import gdal_functions
...
def makeWPSfile(tile_extent, dest_file, source, func_ident, filter, func_params, tile, logs):
    ...
    #***log-output
    functions.writeToLog('Make WPS-Execute-File for...\n\t...tile: %s \n\t...tile_extent: %s ' %(str(tile), str(tile_extent)), logs)
    functions.writeToLog( str(result) + ' ' + geometry_type + '(s) were processed in '+ str(round(time.time()-start_time, 3))  +' seconds!',logs)
    #***
...
def doWPSProcess(params):
  ...
  #      while failure == 1:
           # try:
                ...
                functions.writeToLog( 'Processing of: %s with: %s %s(s) took %s seconds!' % (dest_file, test, geometry_type, str(round(time.time()-start_time, 3))),logs_folder)
